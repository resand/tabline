...
The extension makes outbound network requests only in three narrow cases:
...
2. **External wallpaper URL or external video URL** — only if you, the user, explicitly paste one as a background source.
3. **Curated wallpaper and video assets** bundled with the extension itself.
...
- **`tabs`** — used exclusively by two right-click context-menu actions on your saved shortcuts: *Open in new tab* (`chrome.tabs.create`) so the shortcut can also open `chrome://` system pages, and *Open in incognito* (`chrome.windows.create`). The extension does not read tab content, history or any browsing data.
...
The extension does not integrate with any third-party analytics, advertising, A/B testing or telemetry service.
